...
pub fn final_exponentiation<F: RichField + Extendable<D>, const D: usize>(
    ...
    let result = result.mul(builder, &t0);

    // fp12Conjugate(&t[0], f)                              let result = z;
    // e.fp12.inverse(f, f)                                 let t0 = z.conjugate();
    // e.fp12.mulAssign(&t[0], f)                           let result = result.invert();
    // f.set(&t[0])                                         let t0 = t0.mul(result);
    // e.fp12.frobeniusMap2(f)                              let result = t0.frobenius_square();
    // e.fp12.mulAssign(f, &t[0])                           let result = result.mul(t0);
...
pub fn x<F: RichField + Extendable<D>, const D: usize>(builder: &mut CircuitBuilder<F, D>) {
    // z = u + 1
    // z ^ ((p ^ 0 - 1) / 6)
    let z: Fq2Target<F, D> = Fq2Target::new(vec![
